...
const Edit = (props) => {
    ...
    const [moviesName, setMoviesName] = useState(props.data.moviesName);
    ...
    const onSubmit = () => {
        ...
        const json = JSON.stringify({
            "id": props.data.id,
            "moviesName": moviesName == null ? props.data.moviesName : moviesName,
    ...
    return (
        <span className="d-inline-block mb-2 mr-2">
                <Modal isOpen={props.modal} toggle={props.toggle} className={props.className}>
                    <ModalHeader toggle={props.toggle}>Edit Movies Data</ModalHeader>
                    <ModalBody>
                        <Card className="main-card mb-3">
                                    <CardBody>
                                        <CardTitle>Input Movies Data</CardTitle>
                                        <Form>
                                <FormGroup>
                                    <Label for="moviesName">Movies Name </Label>
                                    <Input type="text" name="moviesName" id="moviesName"
                                           placeholder={props.data.moviesName}
                                ...
                                <FormGroup>
                                    <Label for="idCategory">Category Movies</Label>
                                    <Select name="idCategory" id="idCategory"
                                            options={selectOptions}
                                            placeholder={props.data.categoryMovies}
