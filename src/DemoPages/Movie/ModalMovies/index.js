...
const ModalAja = (props) => {
    ...
    const onSubmit = (e) => {
        ...
        const json = JSON.stringify({
            "id": this.props.id,
            "moviesName": moviesName == null ? this.props.moviesName : moviesName,
    ...
    const getOptions = async () => {
        const res = await axios.get('http://localhost:1818/api/moviescategory', {
    ...
    useEffect(() => {
        axios.get('http://localhost:1818/api/movies/' + props.idPro).then(res => {
            setData(res.data)
    ...
    return (
        <span className="d-inline-block mb-2 mr-2">
                <Modal isOpen={props.modal} toggle={props.toggle} className={props.className}>
                    <ModalHeader toggle={props.toggle}>Edit Movies</ModalHeader>
                    <ModalBody>
                                        ...
                                        <Form>
                                            <FormGroup>
                                                ...
                                                <Input type="text" name="name" id="name"
                                                        placeholder={data.moviesName}
                                                       onChange={(e)=>{setMoviesName(e.value)}}/>
                                            </FormGroup>
                                            <FormGroup>
                                                <Label for="category">Category Movies</Label>
                                                <Select name="idCategory" id="idCategory" placeholder={data.categoryMovies}
